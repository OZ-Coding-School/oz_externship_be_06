    ...
    create=extend_schema(
        ...
        responses={
            201: inline_serializer(
                ...
                fields={'detail': CharField(), 'pk': IntegerField()}
            ),
            400: inline_serializer(
                name='PostErrorResponse',
                fields={'error_detail': Any}
            )
        }
    ),
    update=extend_schema(
        summary="게시글 수정",
        description="게시글의 정보를 수정합니다. 작성자만 가능합니다.",
        responses={
            200: inline_serializer(
                name='PostUpdateResponse',
                fields={'id': IntegerField(), 'title': CharField(), 'content': CharField(), 'category_id': IntegerField()}
            ),
            400: inline_serializer(
                name='PostUpdateErrorResponse',
                fields={'error_detail': Any}
            )
        }
    ),
...
class PostViewSet(viewsets.ModelViewSet[Post]):
    ...
    def create(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        return Response({"detail": "게시글이 성공적으로 등록되었습니다.", "pk": serializer.instance.id}, status=status.HTTP_201_CREATED)
    ...
    def update(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        return Response({"id": instance.id, "title": serializer.data.get("title"), "content": serializer.data.get("content"), "category_id": serializer.data.get("category_id")}, status=status.HTTP_200_OK)
