...
class AdminStudentEnrollmentAcceptAPITest(TestCase):
    """어드민 수강생 등록 요청 승인 API 테스트."""
    ...
    def _get_url(self) -> str:
        return "/api/v1/admin/student-enrollments/accept"

    def _auth_headers(self, user: User) -> Any:
        token = AccessToken.for_user(user)
        return {"HTTP_AUTHORIZATION": f"Bearer {token}"}

    def test_admin_can_accept_enrollments(self) -> None:
        """관리자가 수강생 등록 요청을 승인할 수 있다."""
        ...
        self.assertEqual(
            ...
            "수강생 등록 신청들에 대한 승인 요청이 처리되었습니다.",
        )

        # 등록 요청 상태 확인
        self.enrollment1.refresh_from_db()
        self.assertEqual(self.enrollment1.status, StudentEnrollmentRequest.Status.APPROVED)
        self.assertIsNotNone(self.enrollment1.accepted_at)

        # 유저 role 확인
        self.user1.refresh_from_db()
        self.assertEqual(self.user1.role, User.Role.STUDENT)

        # CohortStudent 생성 확인
        self.assertTrue(CohortStudent.objects.filter(user=self.user1, cohort=self.cohort).exists())

    def test_ta_can_accept_enrollments(self) -> None:
        """조교가 수강생 등록 요청을 승인할 수 있다."""
        response = self.client.post(
            self._get_url(),
            data={"enrollments": [self.enrollment1.id]},
            content_type="application/json",
    ...
    def test_can_accept_multiple_enrollments(self) -> None:
        """여러 등록 요청을 일괄 승인할 수 있다."""
        response = self.client.post(
            self._get_url(),
        ...
        self.assertEqual(self.enrollment1.status, StudentEnrollmentRequest.Status.APPROVED)
        self.assertEqual(self.enrollment2.status, StudentEnrollmentRequest.Status.APPROVED)

        # 두 유저 모두 STUDENT로 변경됨
        self.user1.refresh_from_db()
        self.user2.refresh_from_db()
        self.assertEqual(self.user1.role, User.Role.STUDENT)
        self.assertEqual(self.user2.role, User.Role.STUDENT)
    ...
    def test_cohort_student_not_duplicated(self) -> None:
        """이미 CohortStudent가 존재하면 중복 생성되지 않는다."""
        # 미리 CohortStudent 생성
        CohortStudent.objects.create(user=self.user1, cohort=self.cohort)
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # CohortStudent는 하나만 존재
        self.assertEqual(CohortStudent.objects.filter(user=self.user1, cohort=self.cohort).count(), 1)
